...
int prioritySort(const void *a, const void *b) {
    return ((struct node*)a)->priority - ((struct node*)b)->priority;
}
...
void disp(int nop, int qt) {
    ...
    for (i = 0; ttime <= alltime;) {
        ...
        qsort(b + f, r - f, sizeof(struct node), prioritySort); // Sort based on priority if burst time is same
        j = f;
        if (b[j].btime > qt) {
            c[k] = b[j];
            c[k].btime = qt;
    ...
    printf("\n\n");
    printf("Process  Burst-time(s)  Arrival-time(s)  Priority  Response-time(s)  Waiting-time(s)  Turnaround-time(s)\n");

    for (i = 0; i < nop && a[i].pname != 'i'; i++) {
        if (a[i].pname == '\0')
            break;
        // Adjust the width as needed for each column
        printf("P%-10c", a[i].pname);
        printf("%-15d", a[i].btime);
        printf("%-16d", a[i].atime);
        printf("%-10d", a[i].priority);
